...
int main() {
  ...
  project.load("/mnt/D/Dev/Arduino/Projects/GameBoyStory/res/Mario/Project"); // TODO: last project
  ...
  { // TODO: Default layout
    ...
    bottom.split(MvGuiDockDirection::Left, 0.5).dock("Assets");
  ...
  while (window.isOpen()) {
    ...
    if (Mova::isKeyHeld(MvKey::Ctrl) && Mova::isKeyPressed(MvKey::E)) { // * Export
      std::ofstream(project.path / "data.hpp") << project.exportAll();
    }
